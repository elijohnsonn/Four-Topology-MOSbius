...
T {Scan chain input artificially generated to enable 5T 1x sizing
...
T {Check the outputs of the other amplifiers in one of our 5T 
amplifier configurations in order to see if there is any 
leakage or feedthrough} -1170 -570 0 0 0.4 0.4 {}
...
N -560 -30 -550 -30 {lab=OUT_TELE}
N -560 30 -550 30 {lab=OUT_CS}
N -560 -90 -510 -90 {lab=OUT_5T}
N -560 -150 -550 -150 {lab=OUT_FOLDED}
...
C {lab_pin.sym} -550 -150 0 1 {name=p11 sig_type=std_logic lab=OUT_FOLDED}
...
let iso_folded = 20 * log(five_t_pp / five_t_pp) / log(10)
let iso_tele = 20 * log(tele_pp / five_t_pp) / log(10)
let iso_cs = 20 * log(cs_pp / five_t_pp) / log(10)

print iso_folded iso_tele iso_cs
.endc
"}
...
C {lab_pin.sym} -320 -90 0 1 {name=p4 sig_type=std_logic lab=OUT_5T}
C {lab_pin.sym} -550 -30 0 1 {name=p6 sig_type=std_logic lab=OUT_TELE}
C {lab_pin.sym} -550 30 0 1 {name=p22 sig_type=std_logic lab=OUT_CS}
